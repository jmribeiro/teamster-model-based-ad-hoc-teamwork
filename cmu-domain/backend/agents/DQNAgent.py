...
class DQNAgent(Agent):
    ...
    def update(self, epochs, verbose=True):
        dataset = self._replay_buffer.all
        X, y = self._preprocess(dataset)
        training_losses, training_accuracies, _ = self._network.fit_and_validate(X, y, epochs=epochs, batch_size=X.shape[0], verbose=verbose)
