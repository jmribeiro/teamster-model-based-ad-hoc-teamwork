...
class PLASTICAgent(Agent, ABC):
    ...
    def reinforcement(self, timestep: Timestep):
        ...
        agent_info["belief distribution"] = {}
        for i, m in enumerate(self.priors):
            agent_info["belief distribution"][m.name] = beliefs[i]
        if self.trainable:
            self._total_training_timesteps.value += 1
            additional_info = self._reinforce(timestep) or {}
            for key in additional_info:
                agent_info[key] = additional_info[key]
    ...
    def save_learning_prior(self, directory, team_name, clear=True):
        if not self.learning_prior:
            print("WARN: Not learning any team", flush=True)
            return
        self.learning_prior.name = team_name
        self.learning_prior.save(directory)
        if clear:
    ...
    def learn_team(self, team_name, tmp_directory):
        self.save_learning_prior(tmp_directory, team_name, clear=True)
        self.load_learnt_prior(f"{tmp_directory}/{team_name}", team_name)
